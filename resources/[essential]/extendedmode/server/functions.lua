...
ESX.RegisterCommand = function(name, group, cb, allowConsole, suggestion)
	...
	if ESX.RegisteredCommands[name] then
		print(('[lmodeextended] [^3WARNING^7] An command "%s" is already registered, overriding command'):format(name))
	...
	RegisterCommand(name, function(playerId, args, rawCommand)
		...
		if not command.allowConsole and playerId == 0 then
			print(('[lmodeextended] [^3WARNING^7] %s'):format(_U('commanderror_console')))
		else
			...
			if error then
				if playerId == 0 then
					print(('[lmodeextended] [^3WARNING^7] %s^7'):format(error))
				else
				...
				cb(xPlayer or false, args, function(msg)
					if playerId == 0 then
						print(('[lmodeextended] [^3WARNING^7] %s^7'):format(msg))
					else
...
ESX.TriggerServerCallback = function(name, requestId, source, cb, ...)
	if ESX.ServerCallbacks[name] then
		...
		print(('[lmodeextended] [^3WARNING^7] Server callback "%s" does not exist. Make sure that the server sided file really is loading, an error in that file might cause it to not load.'):format(name))
	end
end
...
ESX.SavePlayer = function(xPlayer, cb)
	...
	print('[Save] Player '..xPlayer.source.." Name : "..GetPlayerName(xPlayer.source).." Saved")
end
...
-- 			if result then
-- 				print('[^2Framework^7] Sauvegarde automatique de tout les joueurs')
-- 			end
-- 		end)
-- 		Wait(30000)
-- 	end
...
ESX.CreatePickup = function(type, name, count, label, playerId, coords, model)
	...
	TriggerClientEvent('esx:createPickup', -1, ""..pickupId.."", label, coords, type, name, model)
	ESX.PickupId = ""..pickupId..""
end

ESX.CreatePickupClothes = function(type, clothe, label, id, coords, model)
	local pickupId = (ESX.PickupIdClothe == 65635 and 0 or ESX.PickupIdClothe + 1)
	ESX.PickupsClothe[""..pickupId..""] = {
		type = type, label = label, coords = coords, id = id
	}

	TriggerClientEvent('esx:createPickupClothe', -1, ""..pickupId.."", label, coords, type, clothe, model)
	ESX.PickupIdClothe = ""..pickupId..""
end

ESX.CreatePickupCards = function(type, infos, label, coords, model)
	local pickupId = (ESX.PickupIdCards == 65635 and 0 or ESX.PickupIdCards + 1)
	ESX.PickupsCards[""..pickupId..""] = {
		type = type, label = label, coords = coords, id = infos
	}

	TriggerClientEvent('esx:createPickupCards', -1, ""..pickupId.."", label, coords, type, infos, model)
	ESX.PickupIdCards = ""..pickupId..""
end

ESX.CreatePickupIdCards = function(type, infos, label, coords, model)
	local pickupId = (ESX.PickupIdIdCards == 65635 and 0 or ESX.PickupIdIdCards + 1)
	ESX.PickupsIdCards[""..pickupId..""] = {
		type = type, label = label, coords = coords, id = infos
	}

	TriggerClientEvent('esx:createPickupIdCards', -1, ""..pickupId.."", label, coords, type, infos, model)
	ESX.PickupIdIdCards = ""..pickupId..""
end
...
ESX.SendWebhook = function(title, message, webhook, color)
	local content = {
        {
        	["color"] = color,
            ["title"] = title,
...
ESX.ExtractIdentifiers = function(src)
    local identifiers = {
        steam = "Aucun",
        ip = "Aucun",
        discord = "Aucun",
