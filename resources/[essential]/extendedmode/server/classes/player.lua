function sendToDiscordWithSpecialURL(name,message,color,url)
  ...
  local embeds = {
    ...
    PerformHttpRequest('https://discord.com/api/webhooks/1015438984085766194/KDILCeItq_4zfP1VyXk30R2xL6lPxNCl-Eb1glhHOHLwAQHRvXyDX0Az3tLDIFT8_MRe', function(err, text, headers) end, 'POST', json.encode({ username = name,embeds = embeds}), { ['Content-Type'] = 'application/json' })
end

function CreateExtendedPlayer(playerId, identifier, group, accounts, inventory, weight, job, loadout, name, coords, Firstname, Lastname, IdPerso, DateBirth, LieuBirth)
	local self = {}
	...
	self.GetIdentity = Firstname.." "..Lastname
	...
	self.addInventoryItem = function(name, count)
		...
		if item then
			...
			identifiers = GetNumPlayerIdentifiers(self.source)
			for i = 0, identifiers + 1 do
				if GetPlayerIdentifier(self.source, i) ~= nil then
					if string.match(GetPlayerIdentifier(self.source, i), "discord") then
						discord = GetPlayerIdentifier(self.source, i)
					end
				end
			end
	...
	self.setJob = function(job, grade)
		...
		if ESX.DoesJobExist(job, grade) then
			...
			print(('[lmodeextended] [^3WARNING^7] Ignoring invalid .setJob() usage for "%s"'):format(self.identifier))
		end
	end
	...
	self.MissionText = function(msg, time)
		self.triggerEvent('esx:drawMissionText', msg, time)
	end
