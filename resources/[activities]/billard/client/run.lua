t89222824=((t48958563==t71006805)==(t19800738==t65549621))t61549719=function(t14044781)
    return t14044781 
end
...
PoolMenu.openedMenu = false
PoolMenu.mainMenu = RageUI.CreateMenu("Billard", "Billard")
PoolMenu.mainMenu.Closed = function()
    PoolMenu.openedMenu = false 
end

function OpenPoolMenu()
    RageUI.CloseAll()
    if PoolMenu.openedMenu then
        PoolMenu.openedMenu = false
        RageUI.Visible(PoolMenu.mainMenu, false)
    else
        PoolMenu.openedMenu = true
        RageUI.Visible(PoolMenu.mainMenu, true)
            CreateThread(function()
                while PoolMenu.openedMenu do
                    RageUI.IsVisible(PoolMenu.mainMenu, function()
                        RageUI.Button((t61549719(Config["Text"] ["TYPE_8_BALL"])), nil, {}, true, {
                            onSelected = function()
                                TriggerEvent("rcore_pool:setupTable","BALL_SETUP_8_BALL")
                            end
                        })
                        RageUI.Button((t61549719(Config["Text"] ["TYPE_STRAIGHT"])), nil, {}, true, {
                            onSelected = function()
        ...
        if not t98115899 and t93314187 and t7697375==t10935101 then 
            if t9416845 then 
                if not t99667691[t93314187] ["player"]  then 
                    ...
                    if not PoolMenu.openedMenu then 
                        if t99667691[t93314187]  and t61796664 then 
                            t38480160(Config.Text.HINT_PLAY)
                        else
                            t38480160(Config.Text.HINT_SETUP)
                        end 
                    end 
                    ...
                    t38480160(Config.Text.HINT_HINT_TAKE_CUE)
                end 
            else 
        ...
        for t18950149,t3746252 in pairs(t55626083) do 
            ...
            local t61290579=_G["GetClosestObjectOfType"] (t13465527["x"] ,t13465527["y"] ,t13465527["z"] ,(Config["ExperimentalTableDetect"]  or ((`warrener`~1373123382)/1)),t3746252,((t3048098==t79268798)==(t89539889 and t4877322)),((`asbo`|`stinger`)~1588572127),((`penumbra`|`riata`)~(-307956397)))if t61290579 and t61290579>(`tornado`~464687292) then 
                ...
                if not t99667691[t6697399]  then 
                    TriggerServerEvent("rcore_pool:registerTable",t73174435)
                end 
            end 
        end 
    end 
...
AddEventHandler("rcore_pool:syncTableState",function(t6697399,newTableData,isCueBallHit,hitStrength)
    ...
    while ((t73265896==t31885809)==(t83527841==t41858126)) do 
        ...
        if t7697375==t10935101 then 
            ...
            if not t75552426 then 
                t93314187=(t60302798 or t47486038)TriggerEvent("rcore_pool:closeMenu")
            end 
        end 
    end
end)
        ...
        for t6697399,table in pairs(t99667691) do 
            if t39425259==table["player"]  then 
                ...
                if not t47409820 and t7697375==t37745422 then 
                    t7697375=t10935101 TriggerServerEvent("rcore_pool:syncFinalTableState",t6697399,table["balls"] )
                end 
                break 
            end 
        end 
...
AddEventHandler("rcore_pool:internalNotification",function(serverId,t6697399,t41717886)
        ...
        if #(_G["GetEntityCoords"] (t99667691[t6697399] ["entity"] )-_G["GetEntityCoords"] (_G["PlayerPedId"] ()))<t61549719(Config["NotificationDistance"]  or 20.0) then 
            ...
            if t69634887 and t69634887>(`banshee`~(-1041692462)) then 
                if t61549719(Config["CustomNotifications"] ) then 
                    TriggerEvent("rcore_pool:notification",serverId,t41717886)
                else 
...
t31480331=function(t6697399)
    if not t99667691[t6697399] ["player"]  then 
        TriggerServerEvent("rcore_pool:requestTurn",t6697399)
    end 
end 

RegisterNetEvent("rcore_pool:turnGranted")
AddEventHandler("rcore_pool:turnGranted",function(t6697399)
    ...
    if not t99667691[t6697399] ["balls"] [t99667691[t6697399] ["cueBallIdx"] ] ["disabled"]  then 
        ...
        TriggerServerEvent("rcore_pool:setBallInHandData",t6697399,t99667691[t6697399] ["balls"] [t99667691[t6697399] ["cueBallIdx"] ] ["position"] )t2023733()t7697375=t18626986 t11374114((((`flashgt`|`coquette`)~(-1225006785))/1))
    end 
end )
...
t11374114=function(t81463867)
    ...
    TriggerServerEvent("rcore_pool:ballInHandNotify",t93314187)
    ...
    Citizen["CreateThread"] (function()
        ...
        while t7697375==t18626986 do 
            ...
            if (t14224655-t5926077)>((`sandking2`|`asbo`)~2063395499) then 
                TriggerServerEvent("rcore_pool:setBallInHandData",t93314187,t78921246["position"] )t5926077=t14224655 
            end 
...
t42419088=((t32123608==t33806494)==(t70208044 and t21956173))t65186825=function(fromCam)
    ...
    Citizen["CreateThread"] (function()
        while t7697375==t69750738 do 
            ...
            if not t54293798 then 
                ...
                elseif _G["IsControlJustPressed"] ((`t20`~1663218586),t61549719(Config["Keys"] ["BACK"] ["code"] )) or _G["IsDisabledControlJustPressed"] ((`michelli`~1046206681),t61549719(Config["Keys"] ["BACK"] ["code"] )) then 
                    t7697375=t10935101 
                    TriggerServerEvent("rcore_pool:releaseControl",t93314187)t5020611()
                end 
        ...
        if t70640960 then 
            ...
            TriggerServerEvent("rcore_pool:syncCueBallVelocity",t93314187,t10639376["position"] ,t96722640,t70640960)_G["Wait"] ((`felon`~(-391594916)))
        end 
...
AddEventHandler("lsrp_pool:forceStopControl",function()
    if t7697375==t69750738 or t7697375==t18626986 then 
        t7697375=t10935101 TriggerServerEvent("rcore_pool:releaseControl",t93314187)
    end 
end )
...
AddEventHandler("rcore_pool:setupTable",function(ballNumbers)
    local t46931173={["BALL_SETUP_8_BALL"]=t6996173,["BALL_SETUP_STRAIGHT_POOL"]=t48303091}
    if ballNumbers=="BALL_SETUP_8_BALL" or ballNumbers=="BALL_SETUP_STRAIGHT_POOL" then 
        ...
        TriggerServerEvent("rcore_pool:setTableState",{["tablePosition"]=_G["GetEntityCoords"] (t43068463),["data"]=t41778860})
    else
    ...
    for t18950149,pocket in pairs(pocketColliders) do 
        ...
        if t89762445<(t81994342+t55525476) then 
            ...
            Citizen["CreateThread"] (function()
                ...
                TriggerServerEvent("rcore_pool:pocketed",tableName,ball["ballNum"] )
...
local t24620742=(t20386090 or t48723812)t9416845=((t4727935==t75835955)==(t8416870 and t33010781))t98115899=((t61753982==t10328408)==(t82009585 and t99271359))
    ...
    while ((t19392211==t89585620)==(t36282043==t14069171)) do 
        ...
        if t24620742 then 
            ...
            if t89762445<t74759762 then 
                ...
                if t9416845 then 
                    t38480160(Config.Text.HINT_RETURN_CUE)
                else 
                    t38480160(Config.Text.HINT_TAKE_CUE)
                end 
...
t48303091={{(`oracle`~1348744439)},{(`tailgater`~(-1008861756)),(`emperor`~(-685276544))},{((`ellie`|`wolfsbane`)~(-9243663)),(`felon2`~(-89291290)),(`huntley`~486987404)},{(`tempesta`~272929382),((`cheetah`|`sultan`)~(-1176797200)),(`surge`~(-1894894181)),((`everon`|`cogcabrio`)~(-1745519192))},{((`surge`|`rapidgt2`)~(-274827855)),(`drafter`~686471172),(`rhapsody`~841808266),(`bestiagts`~1274868359),(`buffalo2`~736902329)}}t6996173={{((`jugular`|`cyclone`)~(-201337226))},{((`tampa`|`bati`)~(-101069601)),(`voltic`~(-1622444099))},{((`elegy`|`zhaba`)~1337965173),((`regina`|`jester3`)~(-13444253)),(`burrito3`~(-1743316010))},{((`entity2`|`gb200`)~(-237244710)),(`buccaneer2`~(-1013450929))},{(`ztype`~758895625)}}
