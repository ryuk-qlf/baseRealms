...
RegisterCommand('+cycleproximity', function()
	...
	-- NotificationVoice('L\'intensité de votre voix a été définie sur '..Cfg.voiceModes[voiceMode][2])

	NotificationVoice('L\'intensité de votre voix a été définie sur '..'~'..Cfg.voiceModes[voiceMode][3]..'~'..Cfg.voiceModes[voiceMode][2].."~s~.")
...
RegisterKeyMapping('+cycleproximity', 'Changez la distance de la voix', 'keyboard', GetConvar('voice_defaultCycle', 'F11'))
	...
	while true do
		...
		if GetConvarInt('voice_enableUi', 1) == 1 then
			if lastRadioStatus ~= radioPressed or lastTalkingStatus ~= (NetworkIsPlayerTalking(PlayerId()) == 1) then
				...
				if Cfg.voiceModes[mode][1] == 3.5 then
					if lastTalkingStatus then
						TriggerEvent("DisplayMic", 'IconNormal', true)
					else
						TriggerEvent("DisplayMic", 'IconNormal', false)
					end
				elseif Cfg.voiceModes[mode][1] == 8.0 then
					if lastTalkingStatus then
						TriggerEvent("DisplayMic", 'IconCrie', true)
					else
						TriggerEvent("DisplayMic", 'IconCrie', false)
					end
				elseif Cfg.voiceModes[mode][1] == 1 then
					if lastTalkingStatus then
						TriggerEvent("DisplayMic", 'IconChuchotement', true)
					else
						TriggerEvent("DisplayMic", 'IconChuchotement', false)
					end
				end
			end
		end
